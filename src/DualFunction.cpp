...
struct DualFunctionWidget : public widgets::XTModuleWidget
{
    ...
    void drawDividingLines(NVGcontext *vg)
    {
        ...
        nvgRect(vg, 5,5,bs.x-10, bs.y-10);
        nvgStrokeWidth(vg, 2);
        nvgStroke(vg);
...
struct ADARCurveDraw : public rack::Widget, style::StyleParticipant
{
    ...
    DualFunction *module{nullptr};
    int adIdx{0};

    std::unordered_map<int, widgets::DirtyHelper<DualFunction>> dirtyChecks;

    static ADARCurveDraw *create(const rack::Vec &pos, const rack::Vec &sz, DualFunction *m, int id)
    {
    ...
    void setup()
    {
        ...
        auto su = [this](auto b, auto m) {
            auto ac = widgets::DirtyHelper<DualFunction>();
            ac.module = module;
            ac.par = b + adIdx;
            ac.isModulated = m;
            dirtyChecks[b] = ac;
        ...
        /*
        su(DualFunction::ATTACK_0, true);
        su(DualFunction::DECAY_0, true);
        su(DualFunction::MODE_0, false);
        su(DualFunction::A_SHAPE_0, false);
        su(DualFunction::D_SHAPE_0, false);
        su(DualFunction::ADAR_0, false);
         */
    }
    ...
    void drawCurves(NVGcontext *vg)
    {
...
#if 0
        auto a = dirtyChecks[DualFunction::ATTACK_0].lastValue;
        auto d = dirtyChecks[DualFunction::DECAY_0].lastValue;
        auto m = dirtyChecks[DualFunction::MODE_0].lastValue;
        auto as = dirtyChecks[DualFunction::A_SHAPE_0].lastValue;
        auto ds = dirtyChecks[DualFunction::D_SHAPE_0].lastValue;
        auto adar = dirtyChecks[DualFunction::ADAR_0].lastValue;
        ...
        auto env = DualFunction::envelope_t(module->storage.get());
